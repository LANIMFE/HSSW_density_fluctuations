Module hd_structure
	...
	Function ck_HD_ros(phi,k) result(ck)
		...
		Real * 8 :: phi2,phi3,phi4,phi1m,phi2m
		Real * 8 :: Z, Zp, chi
		phip = 1.d0 + phi
		phim = 1.d0 - phi
    ...
    !phi3 = phi2 * phi!η³ = η² * η
    !phi4 = phi3 * phi!η⁴ = η³ * η
    !phi1m = 1.d0 / (1.d0-phi)!η₋⁻¹ = 1 / (1 - η)
    !phi2m = phi1m * phi1m!η₋⁻² = η₋⁻¹ * η₋⁻¹

    !Z   = (1 + 0.128d0*phi2 + 0.027d0*phi3 + 0.06d0*phi4) * phi2m	!Z  = (1 + 0.128η² + 0.027η³ + 0.06η⁴) * η₋⁻²
    !Zp  = ((0.256d0*phi + 0.081d0*phi2 + 0.24d0*phi3) * phi2m) + (2d0*Z * phi1m) !((0.256η + 0.081η² + 0.24η³) * η₋⁻²) + (2Z * η₋⁻¹)
    !chi = Z + phi * Zp !χ  = Z + η * Z′

    !g0 = Zp * 0.5d0!G = sqrt(Z′ / 2)
    !a0 = (1.d0 + (2.d0*phi - 1.d0) * chi + 2.d0*phi * g0) / phi !A = (1 + (2η - 1) * χ + 2η * G) / η
    !b0 = (-1.d0 + (1.d0 - phi) * chi - 3.d0*phi * g0) / phi
		If (k>0.075d0) Then
