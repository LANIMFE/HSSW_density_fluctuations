Module structure_function_selector
	...
	Function	structure(F,potential,Approx,fp,qp,qw) result(FR)
		Implicit None
		...
		case("HDAY")
			...
			case("SHVW")
				...
				select case(F)
				case("c0")
					FR = c_hday_shvw(phi,T,z,k,qp,qw)
				case("is")
					FR = is_hday_shvw(phi,T,z,k,qp,qw)
				case("s0")
					FR = s_hday_shvw(phi,T,z,k,qp,qw)
				end select
			!Generalized Sharma-Sharma with Verlet Weiss
			case("GSVW")
				!Parameters
				select case(F)
					case("c0")
						FR = c_hday_gsvw(phi,T,z,k,qp,qw)
					case("is")
						FR = is_hday_gsvw(phi,T,z,k,qp,qw)
					case("s0")
						FR = s_hday_gsvw(phi,T,z,k,qp,qw)
					end select
			end select
		case("HDSW")
			!Parameters:
			...
			case("SHVW")
				...
				select case(F)
				case("c0")
					FR = c_hdsw_shvw(phi,T,lambda,k,qp,qw)
				case("is")
					FR = is_hdsw_shvw(phi,T,lambda,k,qp,qw)
				case("s0")
					FR = s_hdsw_shvw(phi,T,lambda,k,qp,qw)
				end select
			!Generalized Sharma-Sharma with Verlet Weiss
			case("GSVW")
				!Parameters
				select case(F)
					case("c0")
						FR = c_hdsw_gsvw(phi,T,lambda,k,qp,qw)
					case("is")
						FR = is_hdsw_gsvw(phi,T,lambda,k,qp,qw)
					case("s0")
						FR = s_hdsw_gsvw(phi,T,lambda,k,qp,qw)
					end select
			end select
		end select
		return
